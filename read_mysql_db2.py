...
def extract_data(dir_loc, instance):
    ...
    query = db.select(
        [data.c.ID, data.c.NAME, data.c.SCHEMADATA, data.c.LOADERDATA]).where(data.c.SCHEMATYPE == 0)
    ...
    for res in results:
        ...
        # print(res['NAME'])
        try:
            with open(os.path.join('./schemas', str(res['ID']) + '_' + res['NAME']+'.xml'), 'w') as f:
